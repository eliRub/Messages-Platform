...
""" Here I connect to the MySQL server."""
mydb = sql.connect(
...
def is_email_address_valid(u_email: str):
    regex = r'^[A-Za-z0-9._%+-]+@[A-Za-z]+\.[A-Z|a-z]{2,7}$'
...
def show_chat(user1_mail: str, user2_mail: str):
    ...
    for sender, message, date, time in all_messages:
        print(Fore.MAGENTA + f"{sender}: " + Fore.LIGHTYELLOW_EX + message + Fore.RESET)
...
def main():
    ...
    while True:
        ...
        if user_answer == "new chat":
            other_mail = input("Enter other user-mail:  ")
            create_chat(user1_mail=u_mail, user2_mail=other_mail)
        elif user_answer == "my chats":
            reveal_all_user_chas(u_mail)
            ...
            if other_user_mail.lower() == "exit":
                return
            show_chat(u_mail, other_user_mail)
        elif user_answer == "send":
            other_mail = input("Enter other user-mail:  ")
            message = input("Enter Message:  ")
            add_message_to_db(sender_mail=u_mail, receiver_mail=other_mail, message_content=message)
